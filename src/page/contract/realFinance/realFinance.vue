      ...
      <div class="search-container mycontailerclass2">
        <div class="clear">
            ...
            <el-dropdown  placement="bottom">
              <div class="border-icon clear" > 
                ...
                <el-dropdown-menu slot="dropdown" style="margin-left:30px;" >
                  <el-dropdown-item @click.native="quickSearch(1)"   icon="el-icon-plus">全部实收款</el-dropdown-item>
                  <el-dropdown-item @click.native="quickSearch(2)"  icon="el-icon-close" >全部实付款</el-dropdown-item>
                </el-dropdown-menu>
              </div>  
            </el-dropdown>
          </div>
          ...
          <div class="item left clear padding-l-15">
            <div class="border-icon clear" > 
              <i class="back-icon user left" @click="tabRemove('/home/transactionContract')"></i>
            </div>  
          </div>
        </div>
      </div>
        ...
        <el-form ref="formsData" :model="form">
          <div class="clear">
            ...
            <div class="left  padding-l-2" >
              <el-form-item label="" label-width="0">
                <el-select @change="getQuickDate" v-model="quick"  placeholder="" style="width:55px;"   >
            ...
            <div class="left " >
              <el-form-item label="" label-width="0">
                <CheckSelectSpecial :selectId="stateIds" :listData="contratStateParams" style="width:90px;" ></CheckSelectSpecial>
          ...
          <div class="clear position-t-7" >
            <div class="left padding-l-15"   style="width:54px;text-align:right;" >
              <span class="des-word" >财务人</span>
            </div>
            <div class="left" style="width:100px;" >
              <el-form-item >
                <CheckSelectSpecial @findStreetByTown="finUserByDepart" :selectId="departIds" :listData="departParams" style="width:98px;" ></CheckSelectSpecial>
              </el-form-item>
            </div>
            <div class="left padding-l-2" >
              <el-form-item label="" >
                <CheckSelectSpecial :selectId="userId" :listData="userParams" style="width:95px;" ></CheckSelectSpecial>
              </el-form-item>
            </div>
            <div class="left padding-l-15">
            ...
            <div class="left">
              <el-form-item label label-width="0">
                <el-select v-model="form.financeType" placeholder style="width:85px;">
                  <el-option label="全部" value="-1"></el-option>
                  <el-option label="实收" value="2"></el-option>
                  <el-option label="实付" value="3"></el-option>
                </el-select>
              </el-form-item>
            </div>
            <div class="left" style="padding-left:38px;" >
              <span class="des-word">收方</span>
            </div>
            <div class="left padding-l-2">
              <div class="left" >
                <el-form-item label label-width="0">
                  <CheckSelect :selectId="sourceGetIds1" :listData="sourceGetIdParams1" style="width:90px;" ></CheckSelect>
                </el-form-item>
              </div>
            ...
            <div class="left padding-l-2">
              <el-form-item label label-width="0">
                <el-select v-model="form.isAchievement" placeholder style="width:78px;">
                  <el-option label="全部" value="-1"></el-option>
                ...
                <div
                  ...
                  @click="editorRealFinance(scope.$index, scope.row)">
                    <div style="text-algin:center;position:relative;height:100%;" >
                      ...
                      <div v-if="scope.row.beAccount != 0" class="" style="position:absolute;left:-5px;top:6px;">
                        <i :class="{'seven-one-icon':scope.row.beAccount == 1,'seven-two-icon':scope.row.beAccount == 2,'seven-three-icon':scope.row.beAccount == 3}"  ></i>
                      </div>
  ...
  export default {
    data() {
      return {
        ...
        columnData: contractParam.realFinanceColumn,
        upAndDown:1,
        timeDisabled:true,
        departParams:[],
        userParams:[],
    ...
    methods: {
      ...
      quickSearch(type){
        ...
        let form = {};
        if(type ==1 ){
          form = {type:2,financeType:2};
        }else{
          form = {type:2,financeType:3};
        }
        contractFinanceListSpecial(form).then(function(response) {
          ...
          if (response.status == 500) {
            ...
            list.forEach(item => {
              if(item.businessType1 && (item.businessType1).indexOf("X") != -1){
      ...
      getFollowParamData() {
        ...
        contractParams().then(function(response) {
          if (response.status == 500) {
            ...
            _this.contratStateParams = [{id:"-1",name:"全部",select:true}].concat(stateIdParams).concat(finishEndParams).concat(finishSalaryParams);
            _this.getFinanceData();
          } else if (response.status == 1) {
            let message = response.msg;
            _this.myAlert(message + "！");
      ...
      getFinanceData() {
        ...
        let userParams = _this.getSelectIdByList(_this.userParams);
        let contratStateIds = _this.getSelectIdByList(_this.contratStateParams);
        //form.deptIds = departParams;
        //form.userIds = userParams;
            ...
            list.forEach(item => {
              if(item.businessType1 && (item.businessType1).indexOf("X") != -1){
                ...
                if(over != NaN && sourceGetAmount != NaN){
                  if(over == sourceGetAmount ){
                    item.beAccount = 1;
                  }else if(sourceGetAmount > over && over != 0){
                    item.beAccount = 3;
                  }else if(over == 0 && sourceGetAmount != 0){
                    item.beAccount = 2;
                  }
      ...
      exportToExcel() {
        ...
        require.ensure([], () => {
          ...
          export_json_to_excel(tHeader, data, "实收实付");
        });
      },
      ...
      clearForm() {
        ...
        _this.contratStateParams = _this.backSlect(_this.contratStateParams);
      ...
      changeTitle(count,needlayerid,upordown){
        ...
        getLayerId.getElementsByClassName('lv-title')[0].innerHTML = "实收实付修改 【"+_this.tableData[mycountnum].roomAddress+"】";
