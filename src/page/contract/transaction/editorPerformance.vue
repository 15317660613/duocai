<template>
  <div style="width:100%;padding:0 10px;">
    <div class="pop-up" style="margin-bottom: 10px;">
      <div class="search-container" style="height: 270px;">
        <el-form ref="formsData" :model="form">
            ...
            <div class="clear" style>
              ...
              <div class="left padding-r-2">
                <el-form-item prop="departId">
                  <el-select
                    :disabled="type == 2 || finishDisabled"
                    ...
                    style="width:80px;"
                    >
                    <el-option  v-for="item in departments"  :label="item.name" :value="item.id">
              ...
              <div class="left padding-r-2">
                <el-checkbox :disabled="type == 2 || finishDisabled" v-model="form.checked" :false-label="0" :true-label="1" style="padding:1px 0 0 17px;"></el-checkbox>
            ...
            <div class="clear">
              ...
              <div class="left padding-r-2">
                <el-form-item prop="categoryTwo">
                  <el-select :disabled="type == 2 || finishDisabled" @change="getFianceDataByContractId" v-model="form.categoryTwo" placeholder style="width:80px;">
              ...
              <div class="left text-input padding-r-2 des-word">
          ...
          <div class="bottom">
            ...
            <div class="clear">
               ...
               <div class="left padding-r-2" style="padding:0px 0 0 1px;" >
                <el-form-item prop="isCheck" >
                  <el-checkbox
                    ...
                    :disabled="noFinishDisabled  || finishDisabled"
              ...
              <div class="left padding-r-2">
                <el-form-item prop="balanceTime" >
                  <el-date-picker
                    :disabled="noFinishDisabled || finishDisabled || hasCheckBalanceTime"
                    size="mini"
                    type="month"
            ...
            <div class="clear">
             ...
             <div class="break" style="margin:0px 9px 0px"></div>
            <div class="clear" style="padding:20px 0 19px;">
      ...
      <div slot="footer" style="padding:3px 0;" class="dialog-footer clear">
        ...
        <span v-if="!finishDisabled" class="right save-cancel-btn" type="primary" @click="savetBonus" >保 存</span>
...
import { getFinanceByContractId,detailsBonus,updatetBonus,ModelstBonus } from "../../../service/contractData";
import { contractParams ,contractgetByBusinessType2} from "../../../service/contractSpecial";
export default {
  // noFinishDisabled 未结佣标识 ,
  //type 1业绩 2，被同步人业绩
  props: ["businessType","contractId","baTime","businessPersonAuth",'type',"row","noFinishDisabled","layerid","finishDisabled"],
  data() {
    return {
      userParams: [],
      streetArr: [],
  ...
  methods: {
   ...
   checkNum2(){
    ...
    cateChange(){
     ...
     //获取应收应付业绩
    getFianceDataByContractId(init){
      let _this = this;
    ...
    getParam() {
      ...
      let id = this.row.id;
      let form = {id:id};
      detailsBonus(form).then(response => {
        if (response.status == 500) {
          return false;
        } else if (response.status == 0) {
          ...
          if (departTree && departTree.length != 0) {
            ...
            _this.departments = arr;
            
          }
          
          let categoryOneParams = params.categoryOneParams;
          let categoryTwoParams = params.categoryTwoParams;
          let users = response.data.users;
          let models = response.data.models;


          let staffList = params.staffList;
          let departList = params.departList; 
          departList.forEach(item => {
            item.name = item.label;
          })
          _this.departList = departList;
          _this.staffList = staffList;
          _this.industrys = [{id:'',name:' '}].concat(categoryOneParams);
          _this.businessType2Params = [{id:'',name:' ',remark:''}].concat(categoryTwoParams);
          ...
          _this.patternLevels = [{id:'',name:' '}].concat(models);
          let tBonus = response.data.tBonus;
          for(let attr in tBonus){
              if(tBonus[attr] == null){
                  tBonus[attr] = '';
              }
          }
          ...
          _this.form = tBonus;
          let times = response.data.params.times.toString();
          let balanceTime = _this.form.balanceTime;
          let isCalculateSalary = tBonus.isCalculateSalary;
          if(times && times.indexOf(balanceTime) != -1){
            _this.finishDisabled = true;
          }
          if(balanceTime){
              _this.form.isCheck = 1;
          }else{
              _this.form.isCheck = 0;
          }
          if(isCalculateSalary == 1){
              _this.form.isCalculateSalary = 1;
          }else{
              _this.form.isCalculateSalary = 0;
          }
          // 结算年月禁用
          this.changeBalanceTime();
          this.getFianceDataByContractId(true);
        }
      });
    },
    ...
    findUserAndLeaveByDepartId() {
      ...
      if(showLeave){
        form = {id,flag:3,isSelect:1,showLeave:showLeave};
      }
      if (id) {
    ...
    savetBonus() {
      ...
      updatetBonus(form).then(function(response) {
        _this.$layer.closeAll("loading");
        if (response.status == 500) {
          return false;
        } else if (response.status == 1) {
