...
import { statisticsInsertTarget,statisticsDetailTarget } from "../../../service/BusinessStatisticsList";
...
export default {
  props:['myrowdata','mydateType','layerid','myuserType'],
  data() {
    return {
  ...
  methods: {
    setParamsfunc(){
      ...
      if(_this.mydateType==1){
        _this.getActiveName = 'daytime';
        _this.form.yearforday = _this.myrowdata.time.slice(0,4);
        _this.setdayfunc();
        _this.form.date = _this.myrowdata.time.slice(5,7);
        _this.getDayData();
      }else if(_this.mydateType==2){
        _this.getActiveName = 'weektime';
        _this.form.yearforweek = _this.myrowdata.time.slice(0,4);
        _this.getWeekData();
      }else if(_this.mydateType==3){
        _this.getActiveName = 'monthtime';
        _this.form.yearformonth = _this.myrowdata.time.slice(0,4);
        _this.getMonthData();
      }else if(_this.mydateType==4){
        _this.getActiveName = 'seasontime';
        _this.form.yearforseason = _this.myrowdata.time.slice(0,4);
        _this.getSeasonData();
      }else if(_this.mydateType==5){
        _this.getActiveName = 'yeartime';
        _this.form.yearforyear = _this.myrowdata.time.slice(0,4);
        _this.getYearData();
      }
    ...
    getTableData(){
      let _this = this;
      let form = {};
      form.id = _this.myrowdata.id;
      if(_this.mydateType==1){
        form.date = _this.form.yearforday+'-'+_this.form.date;
        form.year = _this.form.yearforday;
        form.dateType = '日';
      }else if(_this.mydateType==2){
        form.date = _this.form.yearforweek;
        form.year = _this.form.yearforweek;
        form.dateType = '周';
      }else if(_this.mydateType==3){
        form.date = _this.form.yearformonth;
        form.dateType = '月';
      }else if(_this.mydateType==4){
        form.date = _this.form.yearforseason;
        form.dateType = '季';
      }else if(_this.mydateType==5){
        form.date = _this.form.yearforyear;
        form.dateType = '年';
      }
      ...
      statisticsDetailTarget(form).then(function(res) {
        _this.$layer.closeAll("loading");
        if (res.status == 500) {
          return false;
        } else if (res.status == 0) {
          let data = res.data.dates[0].childDate;
          if(_this.getActiveName=='daytime'){
            data.forEach(item=>{
              _this.tableDataDay.forEach(ii=>{
                if(ii.day == item.day){
                  for(let i = 1;i<20;i++){
                    if(item['num'+i]){
                      ii['num'+i] = item['num'+i]
          ...
          }else if(_this.getActiveName=='weektime'){
            data.forEach(item=>{
              _this.tableDataWeek.forEach(ii=>{
                if(ii.day == item.day){
                  for(let i = 1;i<20;i++){
                    if(item['num'+i]){
                      ii['num'+i] = item['num'+i]
          ...
          }else if(_this.getActiveName=='monthtime'){
            data.forEach(item=>{
              _this.tableDataMonth.forEach(ii=>{
                if(ii.day == item.day){
                  for(let i = 1;i<20;i++){
                    if(item['num'+i]){
                      ii['num'+i] = item['num'+i]
          ...
          }else if(_this.getActiveName=='seasontime'){
            data.forEach(item=>{
              _this.tableDataSeason.forEach(ii=>{
                if(ii.day == item.day){
                  for(let i = 1;i<20;i++){
                    if(item['num'+i]){
                      ii['num'+i] = item['num'+i]
          ...
          }else if(_this.getActiveName=='yeartime'){
            data.forEach(item=>{
              _this.tableDataYear.forEach(ii=>{
                if(ii.day == item.day){
                  for(let i = 1;i<20;i++){
                    if(item['num'+i]){
                      ii['num'+i] = item['num'+i]
