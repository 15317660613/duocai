<template>
  <div class="" >
    <el-dialog :close-on-click-modal="false" v-dialogDrag  title="新增栋座" width="550px" :visible.sync="dialogNewPedestal" @close="clearFormData" :append-to-body="true" >
      <div class="pop-up" >
              ...
              <div class="clear" >
                <div class="left" >
                  <el-form-item label="栋座名称" prop="name" label-width="68px">
                    <el-input v-model.trim="form.name" maxLength="15" title="上限15个字"   autocomplete="off" style="width:150px" ></el-input>
                  </el-form-item>
                </div>
                <div class="left" >
                  <el-form-item  >
        ...
        <div slot="footer" class="dialog-footer clear">
          <span class="right save-cancel-btn" @click="dialogNewPedestal = false">取 消</span>
...
<script>
  import {edtiorPedestal,newPedestal} from '../../../service/getData'
  import {departperson} from "../../../service/organizeDate";
  export default {
    props:["newPedestalValue"],
    data() {
      ...
      return {
        dialogNewPedestal:this.newPedestalValue,
        stairsParams:[],
        doorNumberParams:[],
        unitParams:[],
        seatNamesParams:[
    ...
    methods:{
      ...
      getSelectData(){
        ...
        edtiorPedestal(id).then(function(response){
          if(response.status == 500){
            ...
            _this.stairsParams = response.data.params.stairsParams;
            let arr1 = [];
            for(let i = 1 ;i <= 200 ; i++){
              arr1.push({id:i,name:i});
            }
            _this.doorNumberParams = arr1;
      ...
      saveFormData(){
        ...
        newPedestal(form).then(function(response){
          if(response.status == 500){
            return false;
          }else if(response.status == 0){
      ...
      saveDataSuccess(){
        ...
        _this.myAlert("栋座保存成功",'success-icon').then(res => {
          ...
          _this.$parent.reloadPedestal(id);
          _this.clearFormData();
        }).catch(err => {
        })
      },
