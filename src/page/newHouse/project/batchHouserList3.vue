    ...
    <div class="pop-up" >
      <div class="pop-up-container" style="margin-top:0px;" >
        ...
        <div class="clear list-container list-content"  >
          <div class="clear" style="padding-bottom:4px;" >
            <div class="left" >
          ...
          <div class="list-table"  style="margin-bottom:10px;">
            <el-table
              :data="tableData"
              style="width: 100%"
              border
            ...
            >
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <div>
                    <el-input :id="'room_'+scope.row.id" maxLength="2"  v-if="scope.row.edite2"  @blur="hideInput(scope.row,2)" @change="changeTableData(scope.row)" v-model="scope.row.room" style="width:100%;hight:100%;"></el-input>
              ...
              <el-table-column
               ...
               >
                <template slot-scope="scope">
                  <div>
                    <el-input :id="'hall_'+scope.row.id" maxLength="2"  v-if="scope.row.edite3"  @blur="hideInput(scope.row,3)" @change="changeTableData(scope.row)" v-model="scope.row.hall" style="width:100%;hight:100%;"></el-input>
              ...
              <el-table-column
               ...
               >
                <template slot-scope="scope">
                  <div>
                    <el-input :id="'wei_'+scope.row.id" maxLength="2"  v-if="scope.row.edite4"  @blur="hideInput(scope.row,4)" @change="changeTableData(scope.row)" v-model="scope.row.wei" style="width:100%;hight:100%;"></el-input>
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <div>
                    <el-input :id="'balcony_'+scope.row.id" maxLength="2" v-if="scope.row.edite5"  @blur="hideInput(scope.row,5)" @change="changeTableData(scope.row)" v-model="scope.row.balcony" style="width:100%;hight:100%;"></el-input>
              ...
              >
                <template slot-scope="scope">
                  <div>
                    <el-input :id="'price_'+scope.row.id" maxLength="10"  v-if="scope.row.edite6" @blur="hideInput(scope.row,6)" @change="changeTableData(scope.row)" v-model="scope.row.price" style="width:100%;hight:100%;"></el-input>
                    <span style="height:20px;text-align:center;display:inline-block;width:75px;border-bottom:1px solid #B6B5B2;" v-else>{{scope.row.price}}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template slot-scope="scope">
                  <div>
                    <el-input :id="'unitPrice_'+scope.row.id" maxLength="10"  v-if="scope.row.edite7" @blur="hideInput(scope.row,7)" @change="changeTableData(scope.row)" v-model="scope.row.unitPrice" style="width:100%;hight:100%;"></el-input>
                    <span style="height:20px;text-align:center;display:inline-block;width:75px;border-bottom:1px solid #B6B5B2;" v-else>{{scope.row.unitPrice}}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template slot-scope="scope">
                  <div>
                    ...
                    >
                      <el-option v-for="item in typesParams"  :label="item.typeName" :value="item.id" ></el-option>
                    </el-select>
              ...
              >
               <template slot-scope="scope">
                  <div>
                    <el-select
                      v-model="scope.row.roomDecorate"
                      @change="changeTableData(scope.row,1)"
...
<script>
  import {findSeatHouseNum,deletedHouseNum,modifyHouseNum}  from '../../../service/newHouse'
  ...
  export default {
    ...
    methods:{
          ...
          if (response.status == 500) {
            ...
            let types = response.data.types;
              for(var i=0;rooms.length>i;i++){
                ...
                if(rooms[i].balcony==-1){
                  rooms[i].balcony=""
                }
                if(rooms[i].orientationName==null){
                  rooms[i].orientationName=""
                }
                if(rooms[i].unitId==null){
                  rooms[i].unitId=""
                }
                if(rooms[i].buildArea==null){
                  rooms[i].buildArea=""
                }
                if(rooms[i].inArea==null){
                  rooms[i].inArea=""
                }
              }
            ...
            let decorateIdParams = _this.decorateIdParams;
            if(decorateIdParams.length == 0){
              _this.decorateIdParams = params.decorateIdParams;
            }

            let roomUseParams = _this.roomUseParams;
            if(roomUseParams.length == 0){
              _this.roomUseParams = params.roomUseParams;
            }
            let typesParams = _this.typesParams;
            if(typesParams.length == 0){
              _this.typesParams = types;
            }
      ...
      showEditorHouseNum(){
        ...
        if(select){
          this.$refs['showModifyHouseNum'].modifyHouseNum(select,projectId);
        }else{
      ...
      showDeleteHouseNum(){
        ...
        if(select){
          ...
          _this.myConfirm("警告：已生成的【房号】删除后将不能还原！", 'question-icon','确定','取消','需要时必须重新生成!','my-message','确定要永久删除选定的房号【'+title+'】吗？').then(res => {
