<template>
  <div class="" >
    <el-dialog :close-on-click-modal="false" v-dialogDrag :title="'修改单元 【'+titlename+'】'" width="550px" :visible.sync="dialogVisible" @close="clearFormData" :append-to-body="true" >
      <div class="pop-up" >
          ...
          <div class="move-content" >
            <el-form ref="formsData"  :model="form">
              <div class="clear" >
                ...
                <div class="left" prop="unitName"  >
                  <el-form-item prop="unitName"  >
                    <el-input v-model="form.unitName" title="上限15个字" maxlength="15"    autocomplete="off" style="width:150px" ></el-input>
                  </el-form-item>
                </div>
                <div class="left  padding-r-2 des-word" style="width:60px;text-align:right;"  >
...
<script>
  import {editorUnit,modifyUnit}  from '../../../service/newHouse'
  export default {
    props:["editorUnitValue"],
    data() {
      return {
        dialogVisible:this.editorUnitValue,
        titlename:'',
    ...
    methods:{
      ...
      getSelectData(){
        ...
        debugger;
        editorUnit(id).then(function(response) {
          if (response.status == 500) {
            return false;
          } else if (response.status == 0) {
            let initArr = [{id:"0",name:" "}];
            let params = response.data.params;
            let form = response.data.unit;
      ...
      saveFormData(){
        ...
        modifyUnit(form).then(function(response){
          if(response.status == 500){
            return false;
          }else if(response.status == 0){
