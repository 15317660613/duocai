  ...
  <div class="" >
      ...
      <div class="pop-up" style="width: 780px;margin: 0 10px;" >
        <div class="pop-up-container" style="height:464px;overflow-y:auto;overflow-x:hidden;position: relative;" >
                ...
                <div class="clear" >
                  <div class="left" >
                    <el-form-item  prop="projectName" style="height: 30px;margin: 3px 0 0 30px;">
                      <el-input style="width: 120px;" v-model="formNA.name"  maxlength="10"  title="部门名称" autocomplete="off"  @input="mydebounce(vm)"></el-input>
                    </el-form-item>
                  </div>
          ...
          <div style="width: 500px;position: absolute;bottom: 0px;right:0;padding-left: 10px;">
...
import { InsuranceMoneymyselfodeparttree,newAddInsuranceMoney,userInsurancegetUserList } from "../../../service/InsuranceMoney"
...
export default {
  ...
  methods: {
    ...
    mydebounce: debounce((vm) => {
      ...
      let thisname = _this.formNA.name;
      console.log(_this.formNA.name);
      odeparttree("常态","主",'',thisname).then( (res) =>{
    ...
    getdepartDaata(){
      ...
      odeparttree("常态","主",'',_this.formNA.name).then( (res) =>{
        _this.$layer.closeAll("loading");
        if (res.status == 500) {
          return false;
        } else if (res.status === 0) {
    ...
    gettreeData(val, ss){
      ...
      if(filterArr.length==_this.tableData.length){
        ...
        userInsurancegetUserList(dataform).then( (res)=> {
          _this.$layer.closeAll('loading');
          if (res.status == 500) {
            return false;
          } else if (res.status == 0) {
    ...
    saveFormData(){
      ...
      newAddInsuranceMoney(_this.formNA).then(res => {
        _this.$layer.closeAll("loading");
        if (res.status == 500) {
          return false;
        } else if (res.status === 0) {
