        ...
        <div class="clear">
          <!-- <label> -->
            <div class="left padding-l-15"  >
              <el-radio  v-model="oneORtwo" :label="1">&nbsp;</el-radio>
            </div>
          ...
          <!-- <label> -->
            <div class="left padding-l-15" >
              <el-radio @change="changeToOtherPage"  v-model="oneORtwo" :label="2">&nbsp;</el-radio>
            </div>
          ...
          <div class="item left padding-l-10 clear"  @click="addInternalKey" >
             <div class="border-icon clear" > 
              <div class="left" style="margin-left: 10px;margin-right: 3px;margin-top: 5px;" >
          ...
          <div class="item left clear padding-l-10">
             <div class="border-icon clear" > 
              <div class="left" style="margin-left: 10px;margin-right: 10px;" >
                <i class="filterchoose-icon" title="过滤选中" @click="filterData" ></i>
              </div>
             </div> 
          </div>
          <div class="left padding-l-10" @click="exportToExcel"  style="margin-right: 10px;">
            <div class="border-icon clear" > 
          ...
          <div class="item left clear" >
            <div class="border-icon clear" > 
              <i class="back-icon user left" @click="tabRemove('/home/InternalKey')" ></i>
            </div>
          </div>
          ...
          <template v-if="item.show" v-for="(item,index) in columnData" >
            ...
            >
              <template slot-scope="scope" style="height: 100%;">
                <div class="findparentclass" style="color:#336699;text-algin:center;cursor: pointer;font-weight: bold;" @click="editorInternalKey(scope.$index, scope.row)" v-if="scope.row.userName">
                  {{ scope.row.userName }}
                </div>
                <div class="findparentclass" style="color:#336699;text-algin:center;cursor: pointer;font-weight: bold;" @click="editorInternalKey(scope.$index, scope.row)" v-else="scope.row.userId">
                  {{ scope.row.userId }}
                </div>
              </template>
            </pl-table-column>
...
<script>
  ...
  import { internalKeyfindListkey,internalKeygetKparam } from "../../../service/InternalKey";
  ...
  import EditorInternalKeyPersonPage from "./EditorInternalKeyperson";
  ...
  export default {
    data() {
      return {
        ...
        tableData:[],
        columnData:InternalKeydata.InternalKeyThead,
        currentPage:'',      
        multipleSelection:[], 
        spanArr:[],
    ...
    methods:{
      ...
      rowspan() {
        this.tableData.forEach((item,index) => {
            if( index === 0){
                ...
                if(this.tableData[index].userId == this.tableData[index-1].userId ){
      ...
      changeToOtherPage(){
        this.$router.push('/home/InternalKeyKey');
      },
      getParamsData(){
        let _this = this;
        _this.$layer.loading({
      ...
      searchThisTableList(){
        ...
        internalKeyfindListkey(form).then(function(response){
          ...
          if (response.status == 500) {
            ...
            _this.tableData = sortByKey(data.klist,'userId');
      ...
      editorInternalKey(index, row){
        ...
        let titletext = '';
        if(this.currentPage.userName){
          titletext = this.currentPage.userName;
        }else{
          titletext = this.currentPage.userId;
        }
        let layerId = _this.$layer.iframe({
          content: {
            content: EditorInternalKeyPersonPage, //传递的组件对象
            parent: this,//当前的vue对象
            data:{myid:row.id,myuserId:row.userId,mytype:row.type,mythisindex:index}//props
          },
          area:['600px','550px'],
          title:"钥匙修改 【" + titletext  + "】",
          shadeClose: false,
          shade: true,//是否显示遮罩
          zIndex:19891110,
          cancel:()=>{//关闭事件
      ...
      filterData(){
        ...
        if(select){
          ...
          this.tableData.forEach(function(item){
            if(select.indexOf(item.userId) != -1){
              arr.push(item);
            }
          })
          this.currentNum = 0;
      ...
      handleSelectionChange(elemCheckbox){
        ...
        elemCheckbox.forEach(item => {
          _this.tableData.forEach(ii=>{
            if(item.userId == ii.userId){
              _this.multipleSelection.push(ii.userId);
            }
          })
        });
      ...
      changeTitle(count,needlayerid,upordown){
        ...
        let titletext = '';
        if( _this.tableData[mycountnum].userName){
          titletext =  _this.tableData[mycountnum].userName;
        }else{
          titletext =  _this.tableData[mycountnum].userId;
        };
        getLayerId.getElementsByClassName('lv-title')[0].innerHTML ="钥匙修改 【" + titletext  + "】";
