<template>
  <div class="" >
    <el-dialog :close-on-click-modal="false" v-dialogDrag :title="'选定房转归属人 【'+titlename+'】'" width="350px" @close="clearFormData" :visible.sync="dialogModifyNum" >
      <div class="pop-up" >
          ...
          <div class="move-content">
            <el-form ref="formsData" :model="form">
              <div class="" style="padding:10px 5px;" >
                ...
                <div  class="clear" >
                  <div class="left" >
                    <span class="word">新归属人3</span>
                  </div>
                  <div class="left" >
                    <el-form-item  prop="depart3" >
                     
                      <el-select @change="findUserByDepart(3)" v-model="form.depart3"   class="select" >
                         <el-option   label="不修改人员" value="-1" ></el-option>
                         <el-option   label="修改为空" value="-2" ></el-option>
                        <el-option  v-for="item in departTree"  :label="item.name" :value="item.id" >
                          <span test :class="{padding1:item.level == 1,padding2:item.level == 2,padding3:item.level == 3,padding4:item.level == 4,padding5:item.level == 5,}"  >
                  ...
                  <div class="left" >
                    <el-form-item  prop="emp3" >
                      <el-select v-model="form.emp3"   class="select" >
...
<script>
  import {followParam,changePeople} from "../../../service/houseData";
  ...
  export default {
    ...
    methods:{
      diolagShow(ids){
        ...
        this.ids = ids;
        this.titlename = ids.split(',').length+'条';
        this.getNumData();
      },
      ...
      findUserByDepart(value){
        ...
        if(id && id != -1 && id != -2) {
          ...
          findUserByDepartId(form).then(function (response) {
           ...
           }else if(id == -1 && value == 3){
            _this[userList] =  [{id:'-1',name:'不修改人员'}];
             _this.form["emp"+value] = '-1';
           }else if(id == -2 && value == 3){
             _this[userList] =  [{id:'-2',name:'修改为空'}];
             _this.form["emp"+value] = '-2';
           }
        }
      ...
      saveData() {
        ...
        let emp3 = _this.form.emp3;
        if(!emp1){
          this.myAlert("转归属人为空！",'dangerous-icon');
          return false;
        }
        changePeople(emp1,emp2,emp3,ids).then(function (response) {
          if (response.status == 500) {
            return false;
          } else if (response.status == 0) {
