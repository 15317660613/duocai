        ...
        <div id="dest1">
          <div id="dest2">
            <div id="dest3" class="clear" >
              <i class="warnning"></i>
              <p style=" float:left;margin:20px 0 20px 5px;"><span  id="" >警告:【职位】删除后将不能还原!<br>确定要永久删除选定的【职位】吗?</span></p>
            </div>
          </div>
        </div>
      </div>
      <div slot="footer" style="padding:5px 0;" class="dialog-footer clear">
        <span class="right save-cancel-btn" @click="deljobVisible=false">取 消</span>
...
<script>
  ...
  export default {
    ...
    methods:{
       showdejob(arr){
         debugger
         this.dearr=arr.toString()
         this.deljobVisible=true
       },
      saveData(){
         let _this=this
          let  deleted=1
          let  ids=this.dearr
         dejobs(deleted, ids).then(function (res) {
           if(res.status==0){
             _this.myAlert("数据保存成功！",'success-icon').then(res => {
                 _this.$emit("search");
                 _this.deljobVisible=false;
              }).catch(err => {
