...
<script>
  import {departcoopdetail,departcoopgetParams,departcoopupdate} from '../../../service/PhysicalManagement'
  export default {
    props:['layerid','thismyid'],
    data(){
      return{
        form:{
          departId:'',
    ...
    methods:{
      getDetailData(){
        ...
        departcoopdetail(_this.getId).then(function(res) {
      ...
      saveFormData(){
        ...
        departcoopupdate(_this.form).then(res => {
          _this.$layer.closeAll("loading");
          if (res.status == 500) {
            return false;
          } else if (res.status === 0) {
